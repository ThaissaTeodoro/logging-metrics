...
def test_json_formatter_exception():
    ...
    try:
        ...
        record = logging.LogRecord(
            "logger", logging.ERROR, __file__, 88, "msg with error", None, sys.exc_info(), func="fail"
        )
        formatter = JSONFormatter()
...
def test_log_spark_dataframe_info_basic(tmp_path):
    ...
    log_spark_dataframe_info(df, logger, name="TestDF", show_schema=True, show_sample=True, sample_rows=2)
